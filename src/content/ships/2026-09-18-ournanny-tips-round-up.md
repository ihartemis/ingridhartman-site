...
- Paying is one flow now. The card's "Pay in Venmo" button and the separate "Record a payment" row are replaced by a single "Pay" sheet: set the amount, pick how you paid, and if it's Venmo the app opens Venmo with that exact total. On return, the confirmation is Yes or Not yet; there's no separate "different amount" step because the amount was decided before Venmo opened.
...
- The paid-up receipt no longer repeats the amount. It says which week is covered and, if you rounded up, "and you rounded up 💛".
